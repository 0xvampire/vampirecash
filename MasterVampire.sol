...
// File: contracts\VampireSToken.sol
...
contract VMPRTOKEN is ERC20("vampire.cash", "VMPR"), Ownable {
    using SafeMath for uint256;
...
contract MasterVampire is Ownable {
    ...
    constructor(
        ...
        IUniswapV2Factory uniswapFactory = IUniswapV2Factory(0xBCfCcbde45cE874adCB698cC183deBcF17952812);
        vampiresBusdPair = IUniswapV2Pair(uniswapFactory.getPair(address(busd), address(vampireS)));
    }
    ...
    function add(Victim _victim, uint256 _victimPoolId, uint256 _rewardPerBlock) public onlyOwner {
    ...
    function pendingVampireS(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accVampireSPerShare = accVampireSPerShare.add(vampiresReward.mul(1e12).div(lpSupply));
        }

        return user.amount.mul(accVampireSPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 blocksToReward = block.number.sub(pool.lastRewardBlock);
        uint256 vampiresReward = blocksToReward.mul(pool.rewardPerBlock);
        vampireS.mint(devAddress, vampiresReward.mul(DEV_FEE_MINT).div(100));
        vampireS.mint(address(this), vampiresReward);
        pool.accVampireSPerShare = pool.accVampireSPerShare.add(vampiresReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeVampireSTransfer(msg.sender, pending);
            }
        }
        
        uint256 amountDevFee = _amount.mul(2).div(100);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(_pid == 0) {
                IERC20(address(vampiresBusdPair)).safeTransfer(address(msg.sender), _amount);
            } else {
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(_pid == 0) {
            IERC20(address(vampiresBusdPair)).safeTransfer(address(msg.sender), user.amount);
        } else {
    ...
    function safeVampireSTransfer(address _to, uint256 _amount) internal {
        uint256 balance = vampireS.balanceOf(address(this));
        if (_amount > balance) {
            vampireS.transfer(_to, balance);
        } else {
            vampireS.transfer(_to, _amount);
        }
    }
    ...
    function drain(uint256 _pid) public {
        ...
        address token0 = vampiresBusdPair.token0();
        (uint reserve0, uint reserve1,) = vampiresBusdPair.getReserves();
        (uint reserveInput, uint reserveOutput) = address(busd) == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
        uint amountOutput = UniswapV2Library.getAmountOut(wethReward, reserveInput, reserveOutput);
        (uint amount0Out, uint amount1Out) = address(busd) == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));

        busd.transfer(address(vampiresBusdPair), wethReward);
        vampiresBusdPair.swap(amount0Out, amount1Out, address(this), new bytes(0));

        vampireS.burn(amountOutput);
    }
}