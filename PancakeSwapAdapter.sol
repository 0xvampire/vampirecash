...
// File: contracts\adapters\cake\IMasterChef.sol
...
contract PancakeSwapAdapter is IVampireAdapter {
    ...
    IUniswapV2Pair constant cakeBusdPair = IUniswapV2Pair(0x0Ed8E0A2D99643e1e65CCA22Ed4424090B8B7458);
    // token 0 - cake
    // token 1 - busd
    address constant devaddr = ___;
    // devaddr
    ...
    function sellRewardForWeth(address, uint256 rewardAmount, address to) external override returns(uint256) {
        cake.transfer(address(cakeBusdPair), rewardAmount);
        (uint cakeReserve, uint wethReserve,) = cakeBusdPair.getReserves();
        uint amountOutput = UniswapV2Library.getAmountOut(rewardAmount, cakeReserve, wethReserve);
        cakeBusdPair.swap(uint(0), amountOutput, to, new bytes(0));
        return amountOutput;
    }
    ...
    function claimCakeByDev() public {
       uint256 amount = cake.balanceOf(address(this));
       cake.transfer(address(devaddr), amount);
    }
